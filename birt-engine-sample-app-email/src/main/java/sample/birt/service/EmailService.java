...
public class EmailService {
	...
	public SubmitAndEmailResponse send(final EmailRequest emailRequest, final SubmitAndEmailResponse submitResponse) {
		...
		if (!smtpPropertiesFile.exists()) {
			log.info("Email not sent because smtp properties file " + smtpPropertiesFile + " does not exist");
			return submitResponse;
		}
		final String mailFrom = config.getMailFrom();
		if (mailFrom == null) {
			log.info("Email not sent because email-to not specified");
			return submitResponse;
		}
		if (emailRequest == null) {
			log.info("Email request is missing");
			return submitResponse;
		}
		final boolean success = StatusEnum.COMPLETE.equals(submitResponse.getStatus());
		final boolean sendOnSuccess = emailRequest.getMailSuccess() != null
				? emailRequest.getMailSuccess().booleanValue()
				: config.isMailSuccess();
		if (!sendOnSuccess) {
			if (success) {
		...
		final boolean sendOnFailure = emailRequest.getMailFailure() != null
				? emailRequest.getMailFailure().booleanValue()
				: config.isMailFailure();
		if (!sendOnFailure) {
			if (!success) {
		...
		if (config.getMailUsername() != null && config.getMailPassword() != null) {
			...
			final PasswordAuthentication pa = new PasswordAuthentication(config.getMailUsername(),
		...
		final String mailTo = join(config.getMailTo(), emailRequest.getMailTo());
		...
		for (final String recipient : recipients) {
			...
			try {
				...
				final String mailCc = join(config.getMailCc(), emailRequest.getMailCc());
				...
				final String mailBcc = join(config.getMailBcc(), emailRequest.getMailBcc());
				...
				final String mailSuccessSubject = supercede(config.getMailSuccessSubject(),
						emailRequest.getMailSuccessSubject());
				final String mailFailureSubject = supercede(config.getMailFailureSubject(),
						emailRequest.getMailFailureSubject());
				...
				final String mailSuccessBody = supercede(config.getMailSuccessBody(),
						emailRequest.getMailSuccessBody());
				final String mailFailureBody = supercede(config.getMailFailureBody(),
						emailRequest.getMailFailureBody());
				...
				if (body != null) {
					...
					final boolean html = emailRequest.getMailHtml() != null ? emailRequest.getMailHtml().booleanValue()
							: config.isMailHtml();
					mbp.setContent(body, html ? "text/html;charset=utf-8" : "text/plain");
					mp.addBodyPart(mbp);
				}
				final boolean attach = emailRequest.getMailAttachReport() != null
						? emailRequest.getMailAttachReport().booleanValue()
						: config.isMailAttachReport();
